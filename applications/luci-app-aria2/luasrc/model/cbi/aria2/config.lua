...
if aria2.https then
	...
	o = s:taboption("rpc", Value, "rpc_certificate", translate("RPC certificate"),
		translate("Use the certificate for RPC server. The certificate must be either"
	...
	o = s:taboption("rpc", Value, "rpc_private_key", translate("RPC private key"),
		translate("Use the private key for RPC server. The private key must be"
...
if aria2.https then
	...
	o = s:taboption("http", Value, "certificate", translate("Certificate"),
		translate("Use the certificate for HTTP server. The certificate must be either"
		.. " in PKCS12 (.p12, .pfx) or in PEM format."))
	o.datatype = "file"

	o = s:taboption("http", Value, "private_key", translate("Private key"),
		translate("Use the private key for HTTP server. The private key must be"
		.. " decrypted and in PEM format."))
	o.datatype = "file"
end
...
o = s:taboption("http", Value, "split", translate("Max number of split"),
	translate("How many threads are used to download a single file."))
o.datatype = "uinteger"
o.placeholder = "10"
...
if aria2.bt then
	...
	o = s:taboption("bt", Flag, "bt_load_saved_metadata", translate("Load save metadata"),
		translate("Before getting torrent metadata from DHT when downloading with magnet link, first try to load saved torrent file."))
	o.enabled = "true"
	o.disabled = "false"
	o.default = "false"
	...
	o = s:taboption("bt", Flag, "check_integrity", translate("Check file integrity"),
		translate("Check file integrity by validating piece hashes or a hash of entire file."))
	o.enabled = "true"
	o.disabled = "false"
	o.default = "false"

	o = s:taboption("bt", Flag, "bt_detach_seed_only", translate("ignore only upload task"),
		translate("ignore only upload task, so that subsequent queued tasks start downloading."))
	o.enabled = "true"
	o.disabled = "false"
	o.default = "false"
	...
	o = s:taboption("bt", Value, "bt_prioritize_piece", translate("Try to download first and last pieces of each file first."),
		"%s %s" % {
			translate("This is useful for previewing files."),
			translate("You can append K or M.")
		})
	o.placeholder = "head=20M,tail=20M"
	...
	o = s:taboption("bt", Value, "peer_agent", translate("Peer agent"),
		translate("Peer node masquerading name"))
	o.placeholder = "Transmission/2.94"
	...
	o = s:taboption("bt", Button, "up_tracker", translate("Update BT tracker"), translate("The update script will run in the background and will automatically restart aria2 after the update is complete."))
	o.inputstyle = "reload"
	o.inputtitle = translate("Update immediately")

	function o.write(self, section)
	     sys.call("/etc/up_trackers.sh > /var/log/up_trackers.log 2>&1 &")
	     sys.call("sleep 5")
...
o = s:taboption("advance", Flag, "disable_ipv6", translate("Disable IPv6"),
	translate("This is useful if you have to use broken DNS and want to avoid terribly"
...
o = s:taboption("advance", ListValue, "file_allocation", translate("File allocation method"),
...
o:value("falloc", translate("Fast allocation"))
o:value("prealloc", translate("Full allocation"))
o:value("trunc", translate("Segmentation allocation"))
