...
en.default = 0
en.rmempty = false
en.description = translate("启用后，路由器自动分流解锁，大部分设备无需设置代理。<br />苹果系列设备需要设置 WIFI/有线代理方式为 自动 ,并安装 CA根证书并信任。")

function en.write(self, section, value)
	if value == "1" then
		sys.init.enable("unblockmusic")
		sys.exec("/etc/init.d/unblockmusic start")
	else
		sys.exec("/etc/init.d/unblockmusic stop")
		sys.init.disable("unblockmusic")
	end
...
local ver = sys.exec("cat /usr/share/UnblockNeteaseMusic/core_ver")
...
o.inputstyle = "reload"
o.write = function()
	sys.exec("bash /usr/share/UnblockNeteaseMusic/update_core.sh 2>&1")
