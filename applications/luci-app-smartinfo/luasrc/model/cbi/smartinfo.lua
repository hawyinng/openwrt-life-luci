...
function enable.write(self, section, value)
	if value == "1" then
		sys.exec("/etc/init.d/smartinfo enable")
		sys.exec("/etc/init.d/smartinfo start")
	else
		sys.exec("/etc/init.d/smartinfo stop")
		sys.exec("/etc/init.d/smartinfo disable")
	end
...
log_path=s:taboption("general",Value,"log_path", translate("Path"))
...
touch_path=s:taboption("general",Value,"touch_path", translate("Path"))
