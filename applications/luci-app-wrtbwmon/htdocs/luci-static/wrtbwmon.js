...
(function () {
    ...
    function getDisplayData(data, totals) {
        var result =
            ...
            createTH('下载速度', {id: 'thDownload'}) +
            createTH('上传速度', {id: 'thUpload'}) +
            createTH('总下载流量', {id: 'thTotalDown'}) +
            createTH('总上传流量', {id: 'thTotalUp'}) +
            createTH('流量合计', {id: 'thTotal'}) +
            createTH('首次活动时间', {id: 'thFirstSeen'}) +
            createTH('最后活动时间', {id: 'thLastSeen'});
        result = createTR(result);
        for (var k = 0; k < data.length; k++) {
            result += data[k][0];
        }
        var totalsRow = createTH('全局');
