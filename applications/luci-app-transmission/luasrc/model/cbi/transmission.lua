...
local webinstalled = luci.model.ipkg.installed("transmission-web-control") or uci:get_first("transmission", "transmission", "web_home")
local button = ""
...
m = Map("transmission", "Transmission" .. state_msg, translate("Transmission daemon is a simple bittorrent client, here you can configure the settings.") .. button)
...
if apply then
	luci.util.exec("/etc/init.d/transmission restart >/dev/null 2>&1")
end
