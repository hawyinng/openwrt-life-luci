...
local s=sys.exec("grep -v !x /usr/share/koolproxy/data/rules/easylistchina.txt | wc -l")
local u=sys.exec("grep -v !x /usr/share/koolproxy/data/rules/fanboy.txt | wc -l")
local p=sys.exec("grep -v !x /usr/share/koolproxy/data/rules/yhosts.txt | wc -l")
...
e.description = translate(string.format("<font color=\"red\"><strong>更新订阅规则与Adblock Plus Hosts</strong></font><br /><font color=\"green\">ABP规则: %s条<br />fanboy规则: %s条<br />yhosts规则: %s条<br />每日规则: %s条<br />自定义规则: %s条<br />Host: %s条</font><br />", s, u, p, q, h, i))
...
e.rmempty=false
e:value(0,translate("不过滤"))
e:value(1,translate("http"))
e:value(2,translate("http + https"))
e:value(3,translate("全部端口"))
