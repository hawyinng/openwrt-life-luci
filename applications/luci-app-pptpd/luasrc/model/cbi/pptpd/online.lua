...
local ps = ut.execi("/bin/busybox top -bn1 | grep '/usr/sbin/pppd'")
for line in ps do
...
-- local ps = ut.execi("sed -n '/## pptpd-blacklist-/p' /etc/firewall.user")
local ps = ut.execi("sed = /etc/firewall.user | sed 'N;s/\\n/:/'")
for line in ps do
...
t:option(DummyValue, "CIP", translate("IP Addresses"))
...
function blacklist.write(self, section)
	...
	if self.map:get(section, "BLACKLIST")==0 then
		ut.execi("echo 'iptables -A input_rule -s %s -p tcp --dport 1723 -j DROP ## pptpd-blacklist-%s' >> /etc/firewall.user" % { CIP, CIP })
		ut.execi("iptables -A input_rule -s %s -p tcp --dport 1723 -j DROP" % { CIP })
		null, self.tag_error[section] = luci.sys.process.signal(self.map:get(section, "PID"), 9)
	else
		ut.execi("sed -i -e '/## pptpd-blacklist-%s/d' /etc/firewall.user" % { CIP })
		ut.execi("iptables -D input_rule -s %s -p tcp --dport 1723 -j DROP" % { CIP })
	end
