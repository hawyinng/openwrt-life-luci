...
local s=sys.exec("head -3 /usr/share/koolproxy/data/rules/koolproxy.txt | grep rules | awk -F' ' '{print $3,$4}'")
local u=sys.exec("head -4 /usr/share/koolproxy/data/rules/koolproxy.txt | grep video | awk -F' ' '{print $3,$4}'")
local p=sys.exec("head -3 /usr/share/koolproxy/data/rules/daily.txt | grep rules | awk -F' ' '{print $3,$4}'")
...
if sys.call("pidof koolproxy >/dev/null") == 0 then
	status = translate("<strong><font color=\"green\">KoolProxy 正在运行</font></strong>")
else
	status = translate("<strong><font color=\"red\">KoolProxy 未运行</font></strong>")
end

o = Map("koolproxy", translate("KoolProxy"), translate("广告拦截器，可以拦截视频广告。"))
...
t:tab("white_weblist",translate("网站白名单"))
...
t:tab("weblist",translate("网站黑名单"))

local i = "/etc/adblocklist/adblock"
e = t:taboption("weblist", TextValue, "adblock_domain")
e.description = translate("这些已加入的网站将强制使用过滤器。请输入网站的域名，每行只能输入一个网站域名。例如google.com。")
e.rows = 28
e.wrap = "off"
e.rmempty = false
...
t:tab("white_iplist",translate("IP白名单"))

local i = "/etc/adblocklist/adbypassip"
e = t:taboption("white_iplist", TextValue, "adbypass_ip")
e.description = translate("这些已加入的ip地址将使用过滤器。请输入ip地址或ip地址段，每行只能输入一个ip地址。例如，112.123.134.145/24或112.123.134.145")
e.rows = 28
e.wrap = "off"
e.rmempty = false
...
e.description = translate("输入每行的自定义规则。")
e.rows = 28
e.wrap = "off"
e.rmempty = false
...
translate("添加到列表中客户端将使用指定规则进行过滤"))
